...
public:
    RequestHandler(const QHttpServerRequest& request, QHttpServerResponder responder)
        : m_request(request), m_responder(std::move(responder)) {}

    void run() override {
        QJsonObject jsonResponse;

        if (m_request.method() == QHttpServerRequest::Method::Get) {
            jsonResponse["message"] = "GET request received";
        } else if (m_request.method() == QHttpServerRequest::Method::Post) {
            QJsonDocument jsonDoc = QJsonDocument::fromJson(m_request.body());
            QJsonObject jsonObject = jsonDoc.object();
            jsonResponse["message"] = "POST request received";
            jsonResponse["data"] = jsonObject;
        } else {
            jsonResponse["error"] = "Unsupported request method";
        }

        QJsonDocument responseDoc(jsonResponse);
        QByteArray responseData = responseDoc.toJson();

	m_responder.write(responseDoc, QHttpServerResponder::StatusCode::Ok);
	// ERASE
        // if (m_responder.isOpen()) {
        //     m_responder.write(QHttpServerResponse(QHttpServerResponse::StatusCode::Ok, responseData, "application/json"));
        // }
    }

private:
    //MEMBERS
    const QHttpServerRequest& m_request;
